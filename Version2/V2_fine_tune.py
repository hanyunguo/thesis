...
from V2_modeling_led_with_memory import LEDForConditionalGenerationWithMemory
...
def preprocess_function(examples):
    ...
    model_inputs["decoder_input_ids"] = shift_tokens_right(
        ...
        decoder_start_token_id=tokenizer.bos_token_id  # 通常是 <s>
    ).tolist()
...
def main():
    ...
    parser.add_argument("--use_memory_attention", action="store_true",
                        help="Enable attention-based MemoryFuse in encoder layers")
    parser.add_argument("--memory_size", type=int, default=128,
                        help="Number of memory slots when using MemoryFuse")
    ...
    base_model = LEDForConditionalGeneration.from_pretrained("allenai/PRIMERA-multinews")
    base_model.config.use_memory_attention = args.use_memory_attention
    base_model.config.memory_size        = args.memory_size
    
    model = LEDForConditionalGenerationWithMemory(base_model.config)
